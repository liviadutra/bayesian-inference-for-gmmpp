...
for(i = 1; i < num_intervalos; ++i){
	...
	aux3[i] = (aux3[i] .<= rows(X)-1 ) .&& (aux3[i] .>= aux3[i-1]) .? aux3[i] .: rows(X)-1;
...
if(num_intervalos.== 1){	  
	...
	if(rows(X_interval) .== 1){	 println("entrou no if de rows(X_interval) == 1");  
	X_new = X_interval;
	}
	else{
	...
	constantes = constantes_func_cond_ultimo(intervalos_tempos_aux, obs_bloco_aux, V_b_cond, R_b_cond, delta,
					psi,y, W_interval, X_interval[][0:1], X_interval[][2], B_matrix, B_matrix[V_b_cond[0]][],
					e, k, valores_r, probs_r, classe_priori_r,
					hiperpar_r);  
...
if(X_new[rows(X_new)-1][1] .== S){	 println("if final");
